...
The outer and inner confidence sets (CSs) for the inverse lower excursion set $ \mu^{-1}(-\infty, c]$ are defined as:
$\text{CS}_{\text{in}}(-\infty, c] := \hat{B}_u^{-1}(-\infty, c]
= \left\{ \hat{B}_u^{-1}[c, +\infty) \right\}^{\complement}$

$\text{CS}_{\text{out}}(-\infty, c] := \hat{B}_\ell^{-1}(-\infty, c]
= \left\{ \hat{B}_\ell^{-1}[c, +\infty) \right\}^{\complement}$
...
1. The **sample mean** $\hat{\mu}_N(s) = \frac{1}{N} \sum_{i=1}^N \tilde{Y}_i(s)$, where \( \tilde{Y}_i(s) \) is the observed functional response.
